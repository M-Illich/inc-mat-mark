...
//		new PrologTest().testProlog(fileNoMark, fileMark, "src/main/prolog/materialize_ways.pl", "src/test/resources/updates_ways");
	...
	public void testCreateInsertRule() {
		Rule r = new Rule(new Atom("p(X,Z)"), List.of(new Atom("e(X,Y)"), new Atom("p(Y,Z)")),
				List.of(new Constraint("X < Y")));
		String exp1 = "phase(4), fact([e, X, Y],add,M1,U1), fact([p, Y, Z],add,M2,U2) ==> X < Y, member(U,[U1,U2]) | check_neg_mark([M1,M2],M), fact([p, X, Z],add,M,U), applied_rules(1,ins).";
		String exp2 = "phase(4), fact([e, X, Y],add,U1), fact([p, Y, Z],add,U2) ==> X < Y, member(U,[U1,U2]) | fact([p, X, Z],add,U), applied_rules(1,ins).";
