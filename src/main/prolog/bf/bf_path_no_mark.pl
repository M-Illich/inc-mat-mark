...
phase(1), fact([edge, X, Y],O1,_) \ fact([path, X, Y],add,U) <=> member(del,[O1]) | fact([path, X, Y],chk,U), applied_rules(1,del).
phase(1), fact([edge, X, Y],O1,_), fact([path, Y, Z],O2,_) \ fact([path, X, Z],add,U) <=> member(del,[O1,O2]) | fact([path, X, Z],chk,U), applied_rules(1,del).
...
% - forward -
fact([edge, X, Y],prv,_) \ fact([path, X, Y],O,U) <=> member(O,[chk,chk1]) | fact([path, X, Y],prv,U), applied_rules(1,fwd).
fact([edge, X, Y],prv,_), fact([path, Y, Z],prv,_) \ fact([path, X, Z],O,U) <=> member(O,[chk,chk1]) | fact([path, X, Z],prv,U), applied_rules(1,fwd).


% - backward -
fact([path, X, Y],chk1,_), fact([edge, X, Y],O1,U1) ==> \+member(del,[O1]) | fact([edge, X, Y],chk1,U1), applied_rules(1,bwd).
fact([path, X, Z],chk1,_), fact([edge, X, Y],O1,U1), fact([path, Y, Z],O2,U2) ==> \+member(del,[O1,O2]) | fact([edge, X, Y],chk1,U1), fact([path, Y, Z],chk1,U2), applied_rules(1,bwd).
