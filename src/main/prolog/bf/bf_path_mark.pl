...
phase(1), fact([edge, X, Y],O1,_,_) \ fact([path, X, Y],add,_,U) <=> member(del,[O1]) | fact([path, X, Y],chk,_,U), applied_rules(1,del).
phase(1), fact([edge, X, Y],O1,_,_), fact([path, Y, Z],O2,_,_) \ fact([path, X, Z],add,_,U) <=> member(del,[O1,O2]) | fact([path, X, Z],chk,_,U), applied_rules(1,del).
...
% - forward -
fact([edge, X, Y],prv,M1,_) \ fact([path, X, Y],O,_,U) <=> member(O,[chk,chk1]) | fact([path, X, Y],prv,M1,U), applied_rules(1,fwd).
fact([edge, X, Y],prv,M1,_), fact([path, Y, Z],prv,M2,_) \ fact([path, X, Z],O,_,U) <=> member(O,[chk,chk1]) | check_neg_mark([(edge,M1),(path,M2)],M), fact([path, X, Z],prv,M,U), applied_rules(1,fwd).


% - backward -
fact([path, X, Y],chk1,_,_), fact([edge, X, Y],O1,M1,U1) ==> \+member(del,[O1]) | fact([edge, X, Y],chk1,M1,U1), applied_rules(1,bwd).
fact([path, X, Z],chk1,_,_), fact([edge, X, Y],O1,M1,U1), fact([path, Y, Z],O2,M2,U2) ==> \+member(del,[O1,O2]) | fact([edge, X, Y],chk1,M1,U1), fact([path, Y, Z],chk1,M2,U2), applied_rules(1,bwd).
...
phase(5), fact([edge, X, Y],add,M1,U1) ==> member(U,[U1]) | fact([path, X, Y],add,M1,U), applied_rules(1,ins).
phase(5), fact([edge, X, Y],add,M1,U1), fact([path, Y, Z],add,M2,U2) ==> member(U,[U1,U2]) | check_neg_mark([(edge,M1),(path,M2)],M), fact([path, X, Z],add,M,U), applied_rules(1,ins).
