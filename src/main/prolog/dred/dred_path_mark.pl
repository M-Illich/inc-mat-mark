...
phase(1), fact([edge, X, Y],O1,M1,_) \ fact([path, X, Y],add,_,U) <=> member(del,[O1]) | fact([path, X, Y],del,M1,U), applied_rules(1,del).
phase(1), fact([edge, X, Y],O1,M1,_), fact([path, Y, Z],O2,M2,_) \ fact([path, X, Z],add,_,U) <=> member(del,[O1,O2]) | check_pos_mark([(edge,O1,M1),(path,O2,M2)],M), fact([path, X, Z],del,M,U), applied_rules(1,del).
phase(1) <=> phase(2).

% -- re-add deleted facts that still have some alternative derivation --
phase(2), fact([edge, X, Y],add,M1,_) \ fact([path, X, Y],del,_,U) <=> true | fact([path, X, Y],add,M1,U), applied_rules(1,red).
phase(2), fact([edge, X, Y],add,M1,_), fact([path, Y, Z],add,M2,_) \ fact([path, X, Z],del,_,U) <=> true | check_neg_mark([M1,M2],M), fact([path, X, Z],add,M,U), applied_rules(1,red).
...
phase(5), fact([edge, X, Y],add,M1,U1), fact([path, Y, Z],add,M2,U2) ==> member(U,[U1,U2]) | check_neg_mark([M1,M2],M), fact([path, X, Z],add,M,U), applied_rules(1,ins).
