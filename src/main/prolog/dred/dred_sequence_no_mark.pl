...
phase(1), fact([edge, X, Y],O1,_) \ fact([edge1, X, Y],add,U) <=> member(del,[O1]) | fact([edge1, X, Y],del,U), applied_rules(1,del).
phase(1), fact([edge1, X, Y],O1,_) \ fact([edge2, X, Y],add,U) <=> member(del,[O1]) | fact([edge2, X, Y],del,U), applied_rules(1,del).
phase(1), fact([edge2, X, Y],O1,_) \ fact([edge3, X, Y],add,U) <=> member(del,[O1]) | fact([edge3, X, Y],del,U), applied_rules(1,del).
phase(1), fact([edge3, X, Y],O1,_) \ fact([edge4, X, Y],add,U) <=> member(del,[O1]) | fact([edge4, X, Y],del,U), applied_rules(1,del).
phase(1) <=> phase(2).

% -- re-add deleted facts that still have some alternative derivation --
phase(2), fact([edge, X, Y],add,_) \ fact([edge1, X, Y],del,U) <=> true | fact([edge1, X, Y],add,U), applied_rules(1,red).
phase(2), fact([edge1, X, Y],add,_) \ fact([edge2, X, Y],del,U) <=> true | fact([edge2, X, Y],add,U), applied_rules(1,red).
phase(2), fact([edge2, X, Y],add,_) \ fact([edge3, X, Y],del,U) <=> true | fact([edge3, X, Y],add,U), applied_rules(1,red).
phase(2), fact([edge3, X, Y],add,_) \ fact([edge4, X, Y],del,U) <=> true | fact([edge4, X, Y],add,U), applied_rules(1,red).
