...
phase(1), fact([e, X, Y],O1,M1,_) \ fact([p, X, Y],add,_,U) <=> member(del,[O1]) | fact([p, X, Y],del,M1,U), applied_rules(1,del).
...
phase(2), fact([e, X, Y],add,M1,_) \ fact([p, X, Y],del,_,U) <=> true | fact([p, X, Y],add,M1,U), applied_rules(1,red).
