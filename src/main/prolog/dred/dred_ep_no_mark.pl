...
phase(1), fact([e, X, Y],O1,_) \ fact([p, X, Y],add,U) <=> member(del,[O1]) | fact([p, X, Y],del,U), applied_rules(1,del).
phase(1), fact([e, X, Y],O1,_), fact([p, Y, Z],O2,_) \ fact([p, X, Z],add,U) <=> member(del,[O1,O2]) | fact([p, X, Z],del,U), applied_rules(1,del).
phase(1) <=> phase(2).
