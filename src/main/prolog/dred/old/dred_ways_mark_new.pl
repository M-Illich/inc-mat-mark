...
phase(1), fact([nextInWay,X1,Y1,Z1],O1,M1,_), fact([nextInWay,X2,Y2,Z2],O2,M2,_) 
\ fact([connection,Z1,Z2],add,_,U) <=> 
	Z1 \== Z2,
	member(del,[O1,O2]),
	(X1 == X2 ; X1 == Y2 ; Y1 == X2 ; Y1 == Y2) |
	check_pos_mark([(nextInWay,O1,M1),(nextInWay,O2,M2)],M),
	fact([connection,Z1,Z2],del,M,U),
	applied_rules(1,del).
	
phase(1), fact([connection,X,Y],O1,M1,_), fact([connection,Y,Z],O2,M2,_) 
\ fact([connection,X,Z],add,_,U) <=> 
	X \== Y,
	member(del,[O1,O2]) |
	check_pos_mark([(connection,O1,M1),(connection,O2,M2)],M),
	fact([connection,X,Z],del,M,U),
	applied_rules(1,del).
